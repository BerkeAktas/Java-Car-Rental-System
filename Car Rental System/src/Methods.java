        ...
        for(Rental<?> i: rentalArrayList) {
            if (i.getType().equals("Individual")) {
                ...
                String customerID = String.valueOf(i.getCustomerID());
                    ...
                    System.out.println(no + "         " + i.getRentalCode() + "       " + i.getCustomerID() + " ".repeat(18-customerID.length()) + isMember +" ".repeat(17-isMember.length())
                            + i.getNumberOfTime() + " ".repeat(13-numberOfDays.length()) + i.getCarModel() + " ".repeat(21-i.getCarModel().length())
                            + i.getCarModelYear() + "           " + i.getDailyPrice() * i.getNumberOfTime());
                    no++;
            }
        }
        System.out.println("\n\nCommercial Rentals:\n");
        no = 1;
        System.out.println("No      Rental Code     Customer ID     isMember    Number of Months      Car Model           Model Year      Rental Price");
        for(Rental<?> i: rentalArrayList) {
            if (i.getType().equals("Commercial")) {
                String isMember = "yes";
                if(i.getMembership().equals("non_member")){
                    isMember = "no";
                }
                String numberOfMonths = String.valueOf(i.getNumberOfTime());
                String customerID = String.valueOf(i.getCustomerID());
                    System.out.println(+no + "         " + i.getRentalCode() + "       " + i.getCustomerID() + " ".repeat(18-customerID.length())  + isMember + " ".repeat(17-isMember.length())
                            + i.getNumberOfTime() + " ".repeat(13-numberOfMonths.length()) + i.getCarModel() + " ".repeat(24-i.getCarModel().length()) + i.getCarModelYear()
                            + "           " + i.getDailyPrice() * i.getNumberOfTime());
                    no++;
            }
        }
    }
